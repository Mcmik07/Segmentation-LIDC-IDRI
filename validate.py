...
def parse_args():
    ...
    parser.add_argument('--name', default="UNET",
                        help='model name: UNET',choices=['UNET', 'NestedUNET','TransUNet'])
...
def main():
    ...
    OUTPUT_MASK_DIR = "D:/Experiment_70/LIDC-IDRI-Segmentation-master/Segmentation_output/{}".format(NAME)
    ...
    ## Load Clean related ####
    ##########################
    CLEAN_DIR_IMG = "data/Clean/Image"
    CLEAN_DIR_MASK ="data/Clean/Mask"
    clean_meta = pd.read_csv("data/Meta/clean_meta.csv")
    # Get train/test label from clean_meta.csv
    clean_meta['original_image'] = clean_meta['original_image'].apply(lambda x: os.path.join(CLEAN_DIR_IMG, x + '.npy'))
    clean_meta['mask_image'] = clean_meta['mask_image'].apply(lambda x: os.path.join(CLEAN_DIR_MASK, x + '.npy'))
    ...
    CLEAN_OUTPUT_MASK_DIR = 'D:/Experiment_70/LIDC-IDRI-Segmentation-master/Segmentation_output/{}'.format(CLEAN_NAME)
